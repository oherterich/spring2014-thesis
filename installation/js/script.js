...
//Big light to illuminate space 
var hemiLight = new THREE.HemisphereLight(0xFFFFFF, 0xFFFFFF, 0.115);
scene.add(hemiLight);
...
function Plane( pic, vel, acc, id, name, time, caption ) {
	this.pic = pic;
	this.vel = vel;
	this.acc = acc;
	this.id = id;
...
for (var i = 0; i < photoLinks[0].length; i++) {
	
	var x = photoLinks[0][i]['posX'] * 1;
	var y = photoLinks[0][i]['posY'] * 1;
	var z = photoLinks[0][i]['posZ'] * 1;
	var rot = photoLinks[0][i]['rot'] * 1;

	var planeGeo = new THREE.PlaneGeometry(imageSize, imageSize, 1, 1);

	//if ( i < 40 ) {
		var photourl = "default_img/" + photoLinks[0][i]['link'] + ".jpg";
	//}
	//else {
	...
	if (request.status != 404) {
		...
		var id = photoLinks[0][i]['link'];

		var name = photoLinks[0][i]['name'];
		var time = photoLinks[0][i]['time'];
		var caption = photoLinks[0][i]['caption'];

		planeList.push( new Plane( plane, vel, acc, id, name, time, caption ) );
		scene.add(plane);
	}
}

// INITIALIZE GROUND PLANES
for (var i = 0; i < photoLinks[1].length; i++) {
	
	var x = photoLinks[1][i]['posX'] * 1;
	var y = photoLinks[1][i]['posY'] * 1;
	var z = photoLinks[1][i]['posZ'] * 1;
	var rot = photoLinks[1][i]['rot'] * 1;

	var planeGeo = new THREE.PlaneGeometry(imageSize, imageSize, 1, 1);

	//if ( i < 40 ) {
		var photourl = "instagram_img/" + photoLinks[1][i]['link'] + ".jpg";
	//}
	//else {
		//var photourl = "instagram_img/" + photoLinks[i]['link'] + ".jpg";
	//}
	...
	if (request.status != 404) {
		...
		var vel = new THREE.Vector3( 0, 0, 0 );
		var acc = new THREE.Vector3( 0, 0, 0 );
		var id = photoLinks[1][i]['link'];

		var name = photoLinks[1][i]['name'];
		var time = photoLinks[1][i]['time'];
		var caption = photoLinks[1][i]['caption'];
...
function checkPicClick( id ) {
	for (var i = 0; i < planeList.length; i++) {
		if ( planeList[i].pic.id == id) {
			...
			metaDataText_name = planeList[selectedImage]['name']; //Set the text to the selected name
			...
			metaDataText_date = planeList[selectedImage]['time']; //Set the text to the selected name
			...
			metaDataText_caption = planeList[selectedImage]['caption']; //Set the text to the selected name
...
function manageSelectedPhotoClick(x, y) {
	...
	//Turn image on right
	if (x > w / 2 + imageSize / 3 && x < w / 2 + outerBoundary + imageSize / 3) {
	...
	//Turn image on left
	else if (x < w / 2 - imageSize / 3 && x > w / 2 - outerBoundary - imageSize / 3) {
...
function tiltImage() {
	if(bIsFront) {
		if (mouse.x > w / 2 + imageSize / 3 && mouse.x < w / 2 + outerBoundary + imageSize / 3) {
		...
		else if (mouse.x < w / 2 - imageSize / 3 && mouse.x > w / 2 - outerBoundary - imageSize / 3) {
			if (planeList[selectedImage].pic.rotation.y > -Math.PI / 12) {
				planeList[selectedImage].pic.rotation.y -= 0.1;
				metaPlane.rotation.y -= 0.1;
				metaData_name.rotation.y -= 0.1;
	...
	//Photo is on back
		...
		//Right side
		if (mouse.x > w / 2 + imageSize / 2 && mouse.x < w / 2 + outerBoundary + imageSize / 3) {
		...
		//Left side
		else if (mouse.x < w / 2 - imageSize / 2 && mouse.x > w / 2 - outerBoundary - imageSize / 3) {
